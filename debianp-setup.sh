#!/bin/bash
# =============================================================================
# debianp-setup.sh — Debian Sid Performance Base Setup
# =============================================================================
# Ausgangslage: Minimale Debian Sid TTY-Installation (mini.iso)
# Umfang: APT-Tuning, i386, Nvidia Open (CUDA Repo), NTSYNC, XanMod Kernel,
#         Fish, Kitty, Starship, Fastfetch, Firefox, Steam, gaming.conf
# =============================================================================
...
[[ $EUID -ne 0 ]] && err "Bitte als root ausführen: sudo bash debianp-setup.sh"
...
echo "  ╚═════╝ ╚══════╝╚═════╝ ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝"
echo -e "${NC}"
echo -e "  ${BOLD}Debian Sid — Performance Base Setup${NC}"
echo -e "  XanMod x64v3 · nvidia-open (CUDA Repo) · Fish · Kitty · Starship · Gaming ENV"
echo ""
...
# lsb_release -sc gibt auf Sid manchmal "sid" zurück — XanMod kennt nur "trixie"
XANMOD_CODENAME=$(lsb_release -sc)
if [[ "$XANMOD_CODENAME" == "sid" ]]; then
    XANMOD_CODENAME="trixie"
fi
echo "deb [signed-by=/etc/apt/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org ${XANMOD_CODENAME} main" \
    | tee /etc/apt/sources.list.d/xanmod-release.list
apt update
log "XanMod Repo aktiviert (${XANMOD_CODENAME})"

info "DKMS Build-Dependencies installieren..."
apt install -y --no-install-recommends \
    ...
    lld \
    llvm
log "DKMS Dependencies installiert"

info "XanMod Kernel x64v3 installieren..."
STOCK_KERNELS=$(dpkg -l 'linux-image-*' 2>/dev/null | awk '/^ii/ {print $2}' | grep -v xanmod || true)
apt install -y linux-xanmod-x64v3
log "XanMod Kernel installiert"

# ── Stock Kernel entfernen ────────────────────────────────────────────────────
if [[ -n "$STOCK_KERNELS" ]]; then
    info "Stock Kernel entfernen..."
    for pkg in $STOCK_KERNELS; do
        info "  Entferne: $pkg"
        apt purge -y "$pkg" || warn "Konnte $pkg nicht entfernen — übersprungen"
    done
    # Dazugehörige Header und Metapakete aufräumen
    apt purge -y \
        linux-headers-amd64 \
        linux-image-amd64 \
        linux-headers-$(uname -r) 2>/dev/null || true
    apt autoremove -y
    log "Stock Kernel entfernt"
else
    warn "Keine Stock Kernel gefunden — übersprungen"
fi
...
# Fish Config — Debian Sid Performance
...
echo -e "${BOLD}${GREEN}  Performance Base-Setup abgeschlossen!${NC}"
echo -e "${BOLD}${GREEN}════════════════════════════════════════════${NC}"
echo ""
...
echo -e "  • Kernel:   ${BOLD}uname -r${NC}  (sollte xanmod zeigen)"
