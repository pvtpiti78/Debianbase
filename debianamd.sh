#!/bin/bash
# =============================================================================
# debian-setup.sh — Debian Sid Base Setup (AMD / GRUB)
# =============================================================================
# Ausgangslage: Minimale Debian Sid TTY-Installation (mini.iso)
# Umfang: APT-Tuning, i386, AMDGPU, Microcode, NTSYNC, Fish,
...
echo "  ╚═════╝ ╚══════╝╚═════╝ ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝"
echo -e "${NC}"
echo -e "  ${BOLD}Debian Sid — Base Setup (AMD Edition)${NC}"
echo -e "  Mesa · GRUB · Fish · Kitty · Starship · Gaming ENV"
echo ""
...
info "Basis-Pakete und Treiber installieren..."
apt install -y \
    git curl wget unzip gpg p7zip-full btop fastfetch bash-completion \
    pciutils usbutils lshw rsync vim nano man-db speech-dispatcher \
    xdg-utils xdg-user-dirs pipewire pipewire-pulse wireplumber \
    power-profiles-daemon hunspell hunspell-de-de hunspell-en-us \
    lsb-release firmware-linux firmware-realtek \
...
cat > "$USER_HOME/.config/fastfetch/config.jsonc" <<'EOF'
{
    ...
    "modules": [
        ...
        "separator",
        { "type": "cpu", "key": "CPU" },
        { "type": "gpu", "key": "GPU", "driverSpecific": true, "format": "{name} [{driver}]" },
        { "type": "memory", "key": "RAM" },
...
apt install -y fonts-noto fonts-noto-cjk fonts-noto-color-emoji fonts-liberation fonts-dejavu
...
log "LACT ${LACT_LATEST} installiert und Service aktiviert"
...
# ── gaming.conf (Environment Variables für AMD) ──────────────────────────────
...
# ── GRUB & OS-Prober Automatisierung ──────────────────────────────────────────
info "GRUB und OS-Prober konfigurieren..."
apt install -y os-prober

# OS-Prober in GRUB aktivieren (wichtig für Multiboot)
sed -i 's/^#GRUB_DISABLE_OS_PROBER=false/GRUB_DISABLE_OS_PROBER=false/' /etc/default/grub

# Kernel-Parameter anpassen (zswap aus, quiet)
if grep -q "^GRUB_CMDLINE_LINUX_DEFAULT=" /etc/default/grub; then
    sed -i 's/^GRUB_CMDLINE_LINUX_DEFAULT=.*/GRUB_CMDLINE_LINUX_DEFAULT="quiet loglevel=3 zswap.enabled=0"/' /etc/default/grub
else
    echo 'GRUB_CMDLINE_LINUX_DEFAULT="quiet loglevel=3 zswap.enabled=0"' >> /etc/default/grub
fi

# GRUB updaten
update-grub
log "GRUB und OS-Prober erfolgreich konfiguriert"
...
echo -e "${BOLD}${GREEN}  Base-Setup (AMD/GRUB) abgeschlossen!${NC}"
echo -e "${BOLD}${GREEN}════════════════════════════════════════════${NC}"
echo ""
...
echo -e "  • AMD GPU:  ${BOLD}lact-cli info${NC} oder ${BOLD}vulkaninfo${NC}"
